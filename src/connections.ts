...
export const addRole = async (): Promise<void> => {
  const departments = await pool.query<Department>('SELECT * FROM department;');
...
export const addEmployee = async (): Promise<void> => {
  const roles = await pool.query<Role>('SELECT id, title FROM role;');
  const employees = await pool.query<Employee>('SELECT id, first_name, last_name FROM employee;');
